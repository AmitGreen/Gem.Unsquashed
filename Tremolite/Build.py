...
def gem():
    ...
    @export
    def OPTIONAL_GROUP(name, pattern = absent):
        if pattern is absent:
            assert name.is_tremolite_name

            return TremoliteGroup(
                       intern_arrange('(?P<%s>%s)?', name.name, name.regular_expression),
                       arrange('OPTIONAL_GROUP(%s)', portray_string(name.name)),
                       name,
                       name,
                   )
        ...
        if name_match(name) is none:
            raise_runtime_error('OPTIONAL_GROUP: invalid group name: %s (expected a python identifier)', name)

        return TremoliteOptionalGroup(
                   intern_arrange('(?P<%s>%s)?', name, pattern.regular_expression),
                   arrange('OPTIONAL_GROUP(%s, %s)', portray_string(name), pattern),
                   name,
                   pattern,
               )
